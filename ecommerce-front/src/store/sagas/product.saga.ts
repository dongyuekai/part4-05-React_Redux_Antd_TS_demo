...
function* handleGetProduct({ sortBy, order, limit }: GetProductAction) {
  let response: resonseType = yield axios.get<Product[]>(`${API}/products`, {
...
function* handleSearchProduct({
  ...
  let response: resonseType = yield axios.get(`${API}/products/search`, {
...
function* handleFilterProduct(action: FilterProductAction) {
  let response: resonseType = yield axios.post(`${API}/products/filter`, action.payload)
...
function* handleGetProductById({ payload }: GetProductByIdAction) {
  let response: resonseType = yield axios.get(`${API}/product/${payload.productId}`)
